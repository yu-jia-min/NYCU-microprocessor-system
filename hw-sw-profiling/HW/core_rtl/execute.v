...
(
    ...
    // 我自己加的
    (* mark_debug = "true", keep = "true" *) output wire [40:0]       counter_all_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]       counter_core_list_find_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]       counter_core_list_reverse_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]       counter_matrix_mul_matrix_bitextract_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]       counter_crcu8_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]       counter_core_state_transition_wire,

    (* mark_debug = "true", keep = "true" *) output wire [40:0]        counter_core_list_find_mem_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]        counter_core_list_reverse_mem_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]        counter_matrix_mul_matrix_bitextract_mem_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]        counter_crcu8_mem_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]        counter_core_state_transition_mem_wire,
    (* mark_debug = "true", keep = "true" *) output wire [40:0]        ismem_wire,

     output reg [40:0] counter_core_list_find_mem,
     output reg [40:0] counter_core_list_reverse_mem,
     output reg [40:0] counter_matrix_mul_matrix_bitextract_mem,
     output reg [40:0] counter_crcu8_mem,
     output reg [40:0] counter_core_state_transition_mem,

    output reg [40:0] counter_all,
    output reg [40:0] ismem,
    output reg [40:0] counter_core_list_find,
    output reg [40:0] counter_core_list_reverse,
    output reg [40:0] counter_matrix_mul_matrix_bitextract,
    output reg [40:0] counter_crcu8,
    output reg [40:0] counter_core_state_transition
...
// 我編輯的
// 要一個counter給總clk數、 5個counter給那5個func (這6個最後都要讓外面用ila看到)
//

initial begin
    
    counter_core_list_find_mem = 0;
    counter_core_list_reverse_mem = 0;
    counter_matrix_mul_matrix_bitextract_mem = 0;
    counter_crcu8_mem = 0;
    counter_core_state_transition_mem = 0;
...
assign counter_all_wire = counter_all;
assign ismem_wire = ismem;
assign counter_core_list_find_wire = counter_core_list_find;
assign counter_core_list_reverse_wire = counter_core_list_reverse;
assign counter_matrix_mul_matrix_bitextract_wire = counter_matrix_mul_matrix_bitextract;
assign counter_crcu8_wire = counter_crcu8;
assign counter_core_state_transition_wire = counter_core_state_transition;

assign counter_core_list_find_mem_wire = counter_core_list_find_mem;
assign counter_core_list_reverse_mem_wire = counter_core_list_reverse_mem;
assign counter_matrix_mul_matrix_bitextract_mem_wire = counter_matrix_mul_matrix_bitextract_mem;
assign counter_crcu8_mem_wire = counter_crcu8_mem;
assign counter_core_state_transition_mem_wire = counter_core_state_transition_mem;
...
always @(posedge clk_i) begin
    if(rst_i)ismem <= 0; 
    if((we_i==1 || re_i==1 ) &&pc_i >= 20'h10c0)ismem<=ismem+1;
end
// 每個clk都去檢查 現在的pc是不是在要觀察的func的pc範圍裏面 如果是就+1
// initial區可以放對外的output
always @(posedge clk_i) begin
    if(pc_i >= 20'h10c0 )
        counter_all <= counter_all+1;
    if(pc_i >= 20'h1f10 && pc_i <= 20'h1f58) begin
        counter_core_list_find<=counter_core_list_find+1;
    end
    if(pc_i >= 20'h1f5c && pc_i <= 20'h1f7c) begin
        counter_core_list_reverse<=counter_core_list_reverse+1;
    end
    if(pc_i >= 20'h28b8 && pc_i <= 20'h2968) begin
        counter_matrix_mul_matrix_bitextract<= counter_matrix_mul_matrix_bitextract+1;
    end
    if(pc_i >= 20'h1adc && pc_i <= 20'h1c08) begin
        counter_crcu8<=counter_crcu8+1;
    end
    if(pc_i >= 20'h2dec && pc_i <= 20'h30fc) begin
        counter_core_state_transition<=counter_core_state_transition+1;
    end


    if((we_i==1 || re_i==1) && pc_i >= 20'h1f10 && pc_i <= 20'h1f58) counter_core_list_find_mem<=counter_core_list_find_mem+1;
    if((we_i==1 || re_i==1) && pc_i >= 20'h1f5c && pc_i <= 20'h1f7c) counter_core_list_reverse_mem<=counter_core_list_reverse_mem+1;
    if((we_i==1 || re_i==1) && pc_i >= 20'h28b8 && pc_i <= 20'h2968) counter_matrix_mul_matrix_bitextract_mem<= counter_matrix_mul_matrix_bitextract_mem+1;
    if((we_i==1 || re_i==1) && pc_i >= 20'h1adc && pc_i <= 20'h1c08) counter_crcu8_mem<=counter_crcu8_mem+1;
    if((we_i==1 || re_i==1) && pc_i >= 20'h2dec && pc_i <= 20'h30fc) counter_core_state_transition_mem<=counter_core_state_transition_mem+1;
