...
int main()
{
    ...
    push_back(&new_convolutional_layer(&ctrl, relu, 12, 12, 5, 5, 3, 12, same, 1, 1, 1, 0, 0)->list, &nn->layers);
    push_back(&new_average_pooling_layer(&ctrl, identity, 8, 8, 12, 2, 2)->list, &nn->layers);
    push_back(&new_fully_connected_layer(&ctrl, relu, 4*4*12, 30, 1)->list, &nn->layers);
    push_back(&new_fully_connected_layer(&ctrl, relu, 30, 10, 0)->list, &nn->layers);

    // Perform hand-written digits recognition tests.
    printf("(2) Perform the hand-written digits recognition test.\n");
    printf("Here, we use a 6-layer CNN neural network model.\n");
